...
class EventsController extends Controller
{
	 ...
	 */
	public function actionCreate($rest=false)
	{
		$model=new Events;
		if(isset($_POST['Ceremony'])) {
			...
			if ($model->save())
			{
				...
				if($rest)
					$this->_sendResponse(400, CJSON::encode(['status' => 'failed', 'errors' => $model->errors]), 'application/json');
		}
		if(!$rest) {
	 ...
	 */
	public function actionUpdate($id,$rest=false)
	{
		$model=$this->loadModel($id);
		if(isset($_POST['Ceremony'])) {
			$model->attributes = $_POST['Ceremony'];
			if ($model->save())
			{
				if(!$rest)
					...
					$this->_sendResponse(200, CJSON::encode(['status' => 'success', 'message' => 'مراسم با موفقیت ویرایش شد.']), 'application/json');
			}else
				if($rest)
					$this->_sendResponse(400, CJSON::encode(['status' => 'failed', 'errors' => $model->errors]), 'application/json');
		}
